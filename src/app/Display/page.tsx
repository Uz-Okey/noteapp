...
export default async function Display() {
    ...
    return (
        <div>
            <Link href='/'>
                <AnimateText text="Create a Note" className="font-serif text-center text-green-500" type="blink" custom={1} />
            </Link>
                ...
                {data.map((item) => (
                    <div key={item.id} className="mt-2 ">
                        ...
                        <div className=" text-black shadow-2xl flex flex-col min-h-80 border border-green-500 gap-4 space-y-3 mx-auto p-3">
                            <h1 className="bg-white underline text-shadow-2xs p-2 rounded-sm font-bold text-2xl text-black">Title: {item.title}
